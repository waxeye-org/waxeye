...
(function() {
  ...
  waxeye = (function() {
    var AST, Continuations, ErrAny, ErrCC, ErrChar, Exp, MachineConfiguration, MachineState, Modes, NonTerminal, ParseError, RawError, Value, WaxeyeParser, nonterminal, updateError;
    /*
    ...
    ParseError = class ParseError {
      constructor(pos1, line1, col1, nt1, chars1) {
        this.pos = pos1;
        this.line = line1;
        this.col = col1;
        this.nt = nt1;
    ...
    RawError = class RawError {
      constructor(pos1, nonterminals, failedChars, currentNT) {
        this.pos = pos1;
    ...
    MachineConfiguration = class MachineConfiguration {
      constructor(type, exp1, pos1, asts1, err1, continuations1, value1) {
        this.type = type;
        this.exp = exp1;
        this.pos = pos1;
        this.asts = asts1;
    ...
    MachineState = class MachineState {
      constructor(type, result1, configuration1) {
        this.type = type;
        this.result = result1;
    ...
    Continuations = class Continuations {
      constructor(type, pos1, expressions1, expression, asts1, err1, mode1, name1, nt1) {
        this.type = type;
        this.pos = pos1;
    ...
    WaxeyeParser = class WaxeyeParser {
      ...
      match(nt, input) {
        ...
        move = function(conf) {
          var asts, c, cc, charClass, e, err, errCcs, errNts, errPos, es, exp, exprs, firstExp, inputCodePoint, isMatch, j, k, kFirst, kRest, len, mode, name, newErr, pos, ref, ref1, ref10, ref11, ref12, ref13, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, restExp, ts, valAsts, value;
          asts = conf.asts;
          pos = conf.pos;
          exp = conf.exp;
          err = conf.err;
          ...
          switch (conf.type) {
            case "EVAL":
              switch (exp.type) {
                ...
                case "CHAR":
                  c = exp.args[0];
                  return MachineState.INTER(MachineConfiguration.APPLY(k, c.length === 1 ? (eof(pos)) || (c !== input[pos]) ? Value.FAIL(updateError(err, pos, new ErrChar(c))) : Value.VAL(pos + 1, arrayPrepend(input[pos], asts), err) : eof(pos + 1) || c[0] !== input[pos] || c[1] !== input[pos + 1] ? Value.FAIL(updateError(err, pos, new ErrChar(c))) : Value.VAL(pos + 2, arrayPrepend(input[pos] + input[pos + 1], asts), err))); // c.length == 2
                case "CHAR_CLASS":
                  cc = exp.args;
                  if (eof(pos)) {
